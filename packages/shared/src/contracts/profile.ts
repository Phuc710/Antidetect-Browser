export const PROFILE_RUNTIME_STATES = [
  'validating', 'waiting', 'acquiring_lock', 'preparing', 'starting',
  'running', 'stopping', 'stopped', 'locked', 'crashed', 'error',
] as const;
...
export const BROWSER_DISTRIBUTIONS = ['chromium', 'chrome', 'edge', 'brave', 'firefox', 'webkit', 'custom'] as const;
...
export type ProfileSyncStatus = 'synced' | 'pending_create' | 'pending_update' | 'pending_delete' | 'syncing' | 'conflict' | 'error';
export type ProfileDeletionState = 'active' | 'pending_delete' | 'trashed' | 'purge_pending' | 'purged';
...
export interface NetworkSafetyPolicy {
  stopIfNetworkUnavailable: boolean;
  stopIfIpChanges: boolean;
  stopIfCountryChanges: boolean;
}
...
export interface CreateProfileInput {
  ...
  architecture?: BrowserArchitecture | undefined;
  proxyId?: string | undefined;
  notes?: string | undefined;
  projectId?: string | undefined;
  tags?: string[] | undefined;
...
export interface ListProfilesInput {
  ...
  status?: ProfileRuntimeState | undefined;
  limit?: number | undefined;
  offset?: number | undefined;
}

export interface ProfileListResult { items: ProfileView[]; total: number; }

export interface ProfilesAPI {
  list(input: ListProfilesInput): Promise<ProfileListResult>;
  get(input: { profileId: string }): Promise<ProfileView>;
  ...
  launch(input: { profileId: string; headless?: boolean | undefined }): Promise<{ sessionId: string }>;
