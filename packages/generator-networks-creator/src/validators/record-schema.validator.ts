...
import { KNOWN_WEBGL_RENDERER_PARTS, KNOWN_OS_FONTS } from '../constants';
            ...
            if (
                ...
                if (parsedUserAgent.os.name) {
                    if (parsedUserAgent.os.name.startsWith('Windows')) {
                        knownOsFonts =
                            KNOWN_OS_FONTS.WINDOWS as unknown as string[];
                    } else if (
                        ['macOS', 'iOS'].includes(parsedUserAgent.os.name)
                    ) {
                        knownOsFonts =
                            KNOWN_OS_FONTS.APPLE as unknown as string[];
                    }
                }
                ...
                browserFingerprint: z.object({
                    ...
                    videoCard: z.object({
                        renderer: z
                            ...
                            .refine((renderer) =>
                                (
                                    KNOWN_WEBGL_RENDERER_PARTS as unknown as string[]
                                ).some((part) => renderer.includes(part)),
                            ),
                        vendor: z.string(),
                    }),
