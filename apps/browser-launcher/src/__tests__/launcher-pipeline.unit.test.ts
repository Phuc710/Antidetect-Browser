...
describe('Browser Launcher Refactored Pipeline - Unit Tests', () => {
  describe('Command Validation', () => {
    ...
    it('rejects null or non-object messages', () => {
      ...
      expect(res.issues[0].message).toContain('Message must be a non-null object.');
    });
    ...
    it('rejects initialize with invalid mode', () => {
      ...
      const res = validator.validate(cmd);
      expect(res.success).toBe(false);
      expect(res.issues.some((i) => i.path === 'payload.applicationMode')).toBe(true);
    });
  });
  ...
  describe('Cookie Parsing and Validation', () => {
    ...
    it('detects missing required fields in individual cookies', () => {
      const res = cookieValidator.parse('[{"name": "foo", "value": "bar"}]'); // missing domain
      expect(res.success).toBe(false);
      expect(res.issues.some((i) => i.path.includes('domain'))).toBe(true);
    });

    it('accepts fully formed valid cookies list', () => {
      const res = cookieValidator.parse('[{"name": "foo", "value": "bar", "domain": "example.com", "path": "/", "secure": true}]');
      expect(res.success).toBe(true);
  ...
  describe('Launch Plan Builder', () => {
    ...
    it('constructs an immutable BrowserLaunchPlan from valid payload', () => {
      ...
      expect(plan.nativeArgs).toContain('--user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36');
  ...
  describe('Session Registry', () => {
    ...
    it('rejects duplicate profileId additions', () => {
      ...
      expect(() => registry.add(mockSession2)).toThrow(/Profile prof-1 is already running/);
    });
  ...
  describe('Cookie Sync Coordinator', () => {
    ...
    it('emits precisely once when cookies do change', async () => {
      ...
      const runtimeMock = {
        getCookies: vi.fn()
          .mockResolvedValueOnce([{ name: 'foo', value: 'bar', domain: 'example.com' }])
          .mockResolvedValueOnce([{ name: 'foo', value: 'baz', domain: 'example.com' }]),
      } as any;
  ...
  describe('Fingerprint Service - Color Scheme Emulation', () => {
    it('does not hardcode preferences to dark mode', async () => {
      ...
      await service.injectIntoContext(contextMock, fingerprintMock, 'console.log("marker");');

      // Emulate media must have been called with light
      expect(pageMock.emulateMedia).toHaveBeenCalledWith({ colorScheme: 'light' });
    });
  });
});
