...
} from '../cookies/cookie-validator.js';
    ...
    build(payload: LaunchProfilePayload): BrowserLaunchPlan {
        ...
        if (!payload.profileId || !payload.sessionId) {
            throw LauncherError.invalidCommand(
                'Launch payload missing identity fields.',
            );
        }
        ...
        if (!cookieValidation.success) {
            throw LauncherError.invalidCommand('Invalid cookie payload.', {
        ...
        const userAgent =
            payload.preparedFingerprint?.fingerprintWithHeaders?.fingerprint
                ?.navigator?.userAgent;
        const language =
            payload.preparedFingerprint?.fingerprintWithHeaders?.fingerprint
                ?.navigator?.language;
        ...
        return {
            ...
            fingerprint: {
                fingerprintWithHeaders:
                    payload.preparedFingerprint.fingerprintWithHeaders,
            ...
            proxy: payload.proxy
                ? {
                      ...
                      ...(payload.proxy.username !== undefined
                          ? { username: payload.proxy.username }
                          : {}),
                      ...(payload.proxy.password !== undefined
